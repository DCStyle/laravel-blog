...
title.addEventListener("input", (event) => {
    title_length.innerHTML =
        "Tối đa 255 ký tự. <span class='current_title_length'>" +
        title.value.length +
        "/255</span>";
...
excerpt.addEventListener("input", (event) => {
    excerpt_length.innerHTML =
        "Tối đa 510 ký tự. <span class='current_excerpt_length'>" +
...
// Optional: Pre-fill content for editing
if (hiddenArea.innerHTML !== '') {
...
async function loadImages() {
    ...
    await fetch(url)
        ...
        .catch(error => {
            console.error('Đã xảy ra lỗi khi tải ảnh:', error);
        })
        .finally(() => {
    ...
    if (imageLoadMode === 'body') {
        input.onchange = async () => {
            ...
            if (/^image\//.test(file.type)) {
                ...
                console.warn("Chỉ có thể tải lên hình ảnh.");
            }
...
async function imageHandler(image) {
    ...
    if (image.size > 10 * 1024 * 1024) {
        if (!existingUl) {
            ...
            liElement.textContent = 'Hình ảnh quá lớn. Kích thước tối đa là 10MB!';
...
window.calculateReadTime = function () {
    ...
    fetch("/dashboard/calculate-read-time", {
        ...
        .then(data => {
            document.querySelector('.reading-time').innerHTML = data + " phút";
        })
        .catch(error => {
            console.error("Lỗi:", error);
            Toast.fire({
...
window.categoriesToggle = function () {
    ...
    if (visibleCategories) {
        ...
        toggleButton.innerHTML = 'Mở rộng <i class="fa-solid fa-chevron-down"></i>';
    } else {
        visibleCategories = true;
        toggleButton.innerHTML = 'Thu nhỏ <i class="fa-solid fa-chevron-up"></i>';
    }
