...
window.getFileInfo = function (directory, name) {
    ...
    if (foundImage.length === 0) {
        fetch(imagePath)
            ...
            .catch(error => {
                console.error('Có vấn đề khi thực hiện yêu cầu:', error);
            })
            .finally(() => {
    ...
    data.used.forEach(item => {
        ...
        if (item.type === "Người dùng") {
            ...
            usageDiv.innerHTML = `
                    ...
                    <div class="info">
                        <div class="type">${item.type}</div>
                        <div class="location${item.location === 'Nội dung' ? ' body': ''}">${item.location}</div>
...
window.deleteImage = function (event) {
    Swal.fire({
        ...
        html: "Bạn có chắc muốn xóa?<p style='font-size: 15px; font-weight: 400; margin-top: 5px;'>Thông tin:<br>Hành động này không được khuyến khích.</p>",
        icon: "warning",
