...
window.detectedAutoSave = function () {
    ...
    Swal.fire({
        title: 'Đã phát hiện lưu tự động!',
        text: "Bạn có muốn tải lại bản lưu hay từ chối?",
        icon: 'info',
...
window.savePost = function(submit = false) {
    let title = document.querySelector('input[name=title]').value,
        ...
        body = document.getElementById('hiddenArea').innerHTML,
    ...
    fetch("/dashboard/posts/" + id, {
        ...
        .then(data => {
            ...
            document.querySelector(".post__create .post_options .auto-save-info").innerHTML = '<i class="fa-solid fa-floppy-disk" aria-hidden="true"></i> Đã lưu: ' + getCurrentTime();
        })
        .catch(error => {
    ...
    fetch('/dashboard/posts/' + id + '/auto-save')
        ...
        .then(data => {
            ...
            if (readTime) {
                ...
                readTimeTextElement.textContent = 'Thời gian đọc:: ';
